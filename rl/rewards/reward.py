...
class RewardConfig:
    ...
    shot_reward: float = 0.01
    max_shot_reward: float = 0.05
...
class RewardTracker:
    ...
    def _shot_reward(self, game: TankGame) -> float:
        # 只奖励实际生成的前几发子弹并限制每局累计奖励，避免无脑连射刷分。
        ...
        remaining = max(self.config.max_shot_reward - self.shot_reward_paid, 0.0)
        reward = min(new_shots * self.config.shot_reward, remaining)
        self.shot_reward_paid += reward
        return reward

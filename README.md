...
├─ renderer.py      Pygame 渲染（地图、坦克、爆炸、底部击杀数），供其他脚本复用
├─ demo.py          键盘持续试玩：一局结束加分后换图
├─ rl/               强化学习（迷宫对战）
...
当前规则：每辆坦克都是独立个体，不存在队伍；坦克只有一条命，任意子弹命中一次就被摧毁；对局在首辆坦克死亡后再继续 3.4 秒，剩余子弹仍可打死另一方，再按存活情况判定胜负或平局。试玩画面会在被摧毁处播放爆炸动画。车身实测长 `0.4559` 格、宽 `0.3389` 格，炮管使用固定旋转矩形显示，长度为 `0.26795` 格、宽度为 `0.091` 格，实测最大线速度为 `1.8622` 格/秒。子弹直径为 `0.09` 格、实测速度为 `2.2738` 格/秒、寿命为 10 秒，最大射击间隔为 `0.234` 秒（约 `4.2735` 发/秒），每辆坦克最多同时保留 5 颗子弹。子弹撞墙会反弹，反弹回来也能击中发射者本人。地图内外墙壁统一使用实测的 `0.0735` 格宽度。
...
`W/S` 控制前进和后退，`A/D` 控制左右转向，`J` 发射；同时保留方向键和空格。一方死亡后仍等待 3.4 秒判定胜负；有胜者则对应积分 +1，然后自动换地图开下一局。左下蓝色、右下紫红色是累计击杀数。`R` 立刻换图（本局不计分），`Esc` 退出。
...
每辆坦克输入完整的 `2×96×96` 像素图（墙体和有效区域），支持每局随机的 `6..9` 行列，每格对应8个像素。动态实体不栅格化：最多两个其他坦克和十五颗子弹都保留连续坐标，并按精确位置从墙体 CNN 特征图双线性取样。实体集合用共享 MLP 和 `sum+max` 汇总；策略使用单个18类联合动作头。模型无 A* 路点、墙距射线、注意力和 LSTM，共约14万参数。

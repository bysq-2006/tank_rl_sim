...
class TankGame:
    ...
    def __init__(self, physics_hz: int = 24, time_limit: float = 90.0, rows: int | None = None, cols: int | None = None) -> None:
        ...
        self.unsafe_shots_by_tank: dict[int, int] = {}  # 预测会反弹击中自身的开火次数。
        self.good_shots_by_tank: dict[int, int] = {}  # 预测轨迹会命中或接近敌人的开火次数。
        self.bad_shots_by_tank: dict[int, int] = {}  # 预测轨迹不能接近敌人的开火次数。
    ...
    def reset(self, seed: int | None = None) -> None:
        ...
        self.unsafe_shots_by_tank = {tank.tank_id: 0 for tank in self.tanks}
        self.good_shots_by_tank = {tank.tank_id: 0 for tank in self.tanks}
        self.bad_shots_by_tank = {tank.tank_id: 0 for tank in self.tanks}
        self.elapsed = 0.0
    ...
    def _physics_tick(self, controls: list[Control], events: list[dict[str, Any]]) -> None:
        ...
        for tank, (throttle, steer, fire) in zip(self.tanks, controls):
            ...
            if fire and tank.cooldown <= 0.0:
                self._fire(tank, self.would_shot_hit_self(tank), self.would_shot_reach_enemy(tank))
    ...
    def would_shot_hit_self(self, tank: Tank) -> bool:
        # 按真实墙体反弹轨迹预演子弹并判断是否会先撞回发射坦克。
        ...
        probe = Bullet(x, y, vx, vy, owner_tank_id=tank.tank_id, bounces=bounces)
        for _ in range(int(math.ceil(self.bullet_lifetime / self.dt))):
            self._move_bullet(probe)
            hit = self._bullet_hit(probe)
            if hit is not None:
                return hit.tank_id == tank.tank_id
            if probe.bounces > self.max_bounces:
                return False
        return False

    def would_shot_reach_enemy(self, tank: Tank) -> bool:
        """按真实反弹轨迹预演子弹，进入敌人扩大的圆形范围就算有效射击。"""
        enemies = [other for other in self.tanks if other.tank_id != tank.tank_id and other.alive]
        if not enemies:
            return False
        offset = self.tank_half_length + self.bullet_radius + 0.04
        ux, uy = math.cos(tank.heading), math.sin(tank.heading)
        ...
        if self._circle_hits_wall(x, y, self.bullet_radius):
            ...
            vx, vy, bounces = -vx, -vy, 1
        probe = Bullet(x, y, vx, vy, owner_tank_id=tank.tank_id, bounces=bounces)
        enemy_radius = math.hypot(self.tank_half_length, self.tank_half_width) + self.bullet_radius + 0.08
        for _ in range(int(math.ceil(self.bullet_lifetime / self.dt))):
            self._move_bullet(probe)
            if any(math.hypot(probe.x - enemy.x, probe.y - enemy.y) <= enemy_radius for enemy in enemies):
                return True
            if self._bullet_hit(probe) is not None or probe.bounces > self.max_bounces:
                return False
        return False

    def _fire(self, tank: Tank, would_hit_self: bool = False, would_hit_enemy: bool = False) -> bool:
        ...
        if would_hit_self:
            self.unsafe_shots_by_tank[tank.tank_id] = self.unsafe_shots_by_tank.get(tank.tank_id, 0) + 1
        elif would_hit_enemy:
            self.good_shots_by_tank[tank.tank_id] = self.good_shots_by_tank.get(tank.tank_id, 0) + 1
        else:
            self.bad_shots_by_tank[tank.tank_id] = self.bad_shots_by_tank.get(tank.tank_id, 0) + 1
